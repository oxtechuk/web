...
@section('title', __('تسجيل الدخول - GR Motors'))
...
<div class="gr-auth-wrapper">
    <div class="gr-auth-card">
        ...
        <div class="gr-auth-form-side">
            <h2 class="gr-auth-title">{{ __('تسجيل الدخول') }}</h2>
            ...
            <form action="{{ route('store.auth.login.post') }}" method="POST">
                @csrf
            ...
            <div class="gr-divider">
                <span>{{ __('ليس لديك حساب مسبقاً؟') }}</span>
            </div>

            <div class="gr-secondary-action">
                <a href="{{ route('store.auth.register') }}" class="gr-btn-outline-gold">
                    {{ __('إنشاء حساب جديد') }}
                </a>
            </div>
        </div>
