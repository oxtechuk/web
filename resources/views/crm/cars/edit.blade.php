                ...
                <div class="car-section mb-4">
                    ...
                    <div class="car-section-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                <label class="form-label">{{ __('العجلات') }}</label>
                                <input type="text" name="specs[wheels]" class="form-control" placeholder="19 inch" value="{{ $car->specs['wheels'] ?? '' }}">
                            </div>
                    ...
                    <div class="car-section-body">
                        ...
                        <div class="d-flex flex-column p-3 rounded-3 mb-3" style="background:#F8F9FC;border:1px solid var(--crm-border);">
                            ...
                            <select name="is_highlighted" class="form-select form-select-sm">
                                ...
                                <option value="coming_soon" {{ $car->is_highlighted == 'coming_soon' ? 'selected' : '' }}>{{ __('قريباً في السوق') }}</option>
                            </select>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <div class="d-flex align-items-center gap-3 mb-2 p-2 rounded-3" style="background:#F8F9FC;border:1px solid var(--crm-border);">
                                    ...
                                    <div>
                                        @if($car->getRawOriginal('thumbnail'))
                                            <div class="fw-bold text-dark" style="font-size:12px;">{{ __('الصورة الحالية (صورة مخصصة)') }}</div>
                                        @else
                                            <div class="fw-bold text-primary" style="font-size:12px;">{{ __('الصورة الحالية (صورة افتراضية من الإعدادات)') }}</div>
                                        @endif
                                        <small class="text-muted">{{ __('ارفع صورة جديدة لاستبدال الصورة الحالية') }}</small>
                                    </div>
                                </div>
                                @endif
                            ...
                            <div class="col-12">
                                ...
                                <div class="d-flex flex-wrap gap-2 mt-2">
                                    ...
                                    <div class="position-relative">
                                        ...
                                        <a href="{{ route('crm.cars.delete-image', $img) }}" class="position-absolute top-0 end-0" style="background:#E30613;color:#fff;border-radius:50%;width:18px;height:18px;display:flex;align-items:center;justify-content:center;font-size:10px;text-decoration:none;" onclick="return confirm('حذف؟')">×</a>
                                    </div>
                                    @endforeach
                                </div>
                                @endif
                    ...
                    <div class="car-section-body">
                        ...
                        <div class="d-flex align-items-center gap-3 p-3 rounded-3 mb-3"
                            ...
                            <div class="d-flex align-items-center justify-content-center rounded-3 flex-shrink-0"
                                 style="width:44px;height:44px;background:linear-gradient(135deg,#EE1E26,#a8151b);">
                                <i class="bi bi-file-earmark-pdf text-white fs-5"></i>
                            </div>
                            <div class="flex-grow-1 overflow-hidden">
                                <div class="fw-bold text-truncate" style="font-size:12px;">
                                    {{ basename($car->spec_file) }}
                                </div>
                                <a href="{{ asset('storage/' . $car->spec_file) }}" target="_blank"
                                   class="text-danger small text-decoration-none">
                                    <i class="bi bi-download me-1"></i>{{ __('تحميل الملف الحالي') }}
                                </a>
                            </div>
                            <div class="form-check form-switch mb-0" title="{{ __('إزالة الملف') }}">
                                <input class="form-check-input" type="checkbox" name="remove_spec_file"
                                       id="removeSpecFile" value="1"
                                       onchange="document.getElementById('specFileHint').style.display = this.checked ? 'block' : 'none';"
                                       style="width:36px;height:20px;cursor:pointer;">
                                <label class="form-check-label text-muted small" for="removeSpecFile">{{ __('حذف') }}</label>
                            </div>
                        </div>
                        <div id="specFileHint" class="alert alert-warning small py-2 mb-2" style="display:none;">
                            <i class="bi bi-exclamation-triangle me-1"></i>{{ __('سيتم حذف الملف الحالي عند الحفظ') }}
                        </div>
                        @endif

                        <div>
                            <label class="form-label fw-bold small text-muted mb-1">
                                {{ $car->spec_file ? __('رفع ملف جديد (اختياري)') : __('رفع ملف المواصفات') }}
                            </label>
                            <input type="file" name="spec_file" id="specFileInput"
                                   class="form-control" accept=".pdf,.doc,.docx,.xls,.xlsx"
                                   onchange="updateSpecFileLabel(this)">
                            <div class="mt-2 text-muted" style="font-size:11px;">
                                <i class="bi bi-info-circle me-1"></i>
                                {{ __('الصيغ المدعومة: PDF, Word, Excel — الحجم الأقصى 10 ميجا') }}
                            </div>
                            <div id="specFileSelected" class="mt-2 small text-success fw-bold" style="display:none;">
                                <i class="bi bi-check-circle me-1"></i><span id="specFileNameDisplay"></span>
                            </div>
                        </div>
                    </div>
                </div>
...
function addColorRow(name = '', hex = '#E30613', existingImg = null, existingImgPath = '') {
...
existingColors.forEach(c => {
    ...
    const hex  = isNew ? c  : (c.hex  || '#E30613');
...
// Spec file label update
function updateSpecFileLabel(input) {
    const display = document.getElementById('specFileSelected');
    const name    = document.getElementById('specFileNameDisplay');
