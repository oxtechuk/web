...
@section('title', __('تعديل سؤال شائع') . ' | AutoCRM')
...
@section('content')
    <div class="container-fluid" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        <div class="mb-4 d-flex align-items-center">
            ...
            <div>
                <h4 class="mb-1 fw-bold">{{ __('تعديل السؤال الشائع') }}</h4>
                <p class="text-muted mb-0 small">{{ __('قم بتعديل بيانات السؤال والجواب باللغتين العربية والإنجليزية') }}</p>
            </div>
        </div>
                        ...
                        <div class="card-body p-4 pt-2">
                            <div class="row g-3">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="question[ar]" class="form-control bg-light border-0 shadow-none py-2" required value="{{ $faq->getTranslation('question', 'ar') }}" placeholder="{{ __('مثال: ما هي شروط تمويل السيارات؟') }}">
                                </div>
                                <div class="col-md-12">
                                    <label class="form-label fw-bold small text-muted">{{ __('السؤال (بالإنجليزية)') }} <span class="text-danger">*</span></label>
                                    <input type="text" name="question[en]" class="form-control bg-light border-0 shadow-none py-2" required value="{{ $faq->getTranslation('question', 'en') }}" placeholder="e.g.: What are the car financing conditions?">
                                </div>
                                <div class="col-md-12">
                                    <label class="form-label fw-bold small text-muted">{{ __('الإجابة (بالعربية)') }} <span class="text-danger">*</span></label>
                                    <textarea name="answer[ar]" class="form-control bg-light border-0 shadow-none" rows="5" required placeholder="{{ __('اكتب الإجابة باللغة العربية هنا...') }}">{{ $faq->getTranslation('answer', 'ar') }}</textarea>
                                </div>
                                <div class="col-md-12">
                                    <label class="form-label fw-bold small text-muted">{{ __('الإجابة (بالإنجليزية)') }} <span class="text-danger">*</span></label>
                                    <textarea name="answer[en]" class="form-control bg-light border-0 shadow-none" rows="5" required placeholder="Write the answer in English here...">{{ $faq->getTranslation('answer', 'en') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card border-0 shadow-sm mb-4 rounded-4">
                        ...
                        <div class="card-body p-4 pt-2">
                            ...
                            <div class="form-check form-switch p-3 bg-light rounded-3">
                                <input class="form-check-input {{ app()->getLocale() == 'ar' ? 'ms-0 me-2 float-none' : '' }}" type="checkbox" name="is_active" value="1" id="isActive" {{ $faq->is_active ? 'checked' : '' }} style="cursor: pointer;">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                        <div class="card-body p-4">
                            <button type="submit" class="btn btn-primary w-100 py-3 fw-bold rounded-3 shadow-sm">
                                <i class="bi bi-check-circle me-1"></i> {{ __('حفظ التعديلات') }}
                            </button>
                        </div>
                    </div>
                </div>
